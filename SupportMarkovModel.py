...
def print_comparative_outcomes(simOutputs_mono, simOutputs_combo):
    ...
    # increase in strokes under combination therapy with respect to mono therapy
    increase_strokes = Stat.DifferenceStatIndp(
        name='Increase in number of strokes',
        x=simOutputs_combo.get_if_developed_stroke(),
        y_ref=simOutputs_mono.get_if_developed_stroke())

    # estimate and CI
    estimate_CI = F.format_estimate_interval(
        estimate=increase_strokes.get_mean(),
        interval=increase_strokes.get_t_CI(alpha=Settings.ALPHA),
        deci=2)
    print("Average increase in number of strokes "
